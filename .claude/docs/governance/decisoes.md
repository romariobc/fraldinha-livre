...
Specs de design ficam em `.claude/docs/features/specs/` (neste repo) e sao a referencia obrigatoria do spec-driven development. Copia de backup mantida no harness de referencia (`dev_flow_create_harness`). Planos/prompts de implementacao ficam em `.claude/docs/features/plans/`.
...
- **Sessoes Haiku:** executam exatamente o que o prompt em `.claude/docs/features/plans/` pede, relatam o que foi feito.
- Toda entrega Haiku so e considerada completa apos revisao aprovada na sessao-mae.
- Cada sessao significativa e documentada em `.claude/context/chatsessions/` e o estado em `.claude/context/estado/` e atualizado antes de encerrar.
...
Template padrao em `.claude/docs/features/plans/README.md`.
...
- Spec da feature 014: `.claude/docs/features/specs/spec-catalogo-compra-direta.md`.

## D-012 — Checklist de revisao pre-aprovacao obrigatorio (2026-07-02) — VIGENTE

Toda entrega de sessao Haiku passa pelo `.claude/docs/governance/review-checklist.md` ANTES de a sessao-mae
...
`.claude/docs/architecture/blueprint-arquitetura-2026-07-29.html`) e ataque aos pontos
...
## D-042 — Painel do Fornecedor B2B — Design e Arquitetura (2026-08-15) — VIGENTE
...
## D-043 — Integração ERP — Hub de Fornecedores (2026-08-16) — PENDENTE APROVAÇÃO
...
**Campo `erpId` já existe:** O schema Zod `ProductAtributosSchema` já includes `erpId?: string`, preparado para uso.
...
| 3 | **MCP Catalog Mapping** | Tempos / Fase 1 (antecipado) / Fase 2 (recomendado) | Fase 2 |
...
## D-044 — Seed Real de Produtos no D1 (Milestone 6) (2026-08-17) — VIGENTE
...
## D-045 — Arquitetura de Rotas do Marketplace (Storefront vs Painel B2B) (2026-08-20) — VIGENTE
