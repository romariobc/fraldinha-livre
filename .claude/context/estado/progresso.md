## Marco (2026-08-20) - Reestrutura��o de Rotas (RBAC Blindado)

**Resumo da Sess�o:**
Aplica��o da padroniza��o de grupos de rota (Route Groups) para garantir o isolamento autom�tico de autoriza��o.

**O que foi feito:**
1. **Moveu /sacola, /checkout e /minha-conta:** para o novo grupo (comprador), o qual possui um layout.tsx que injeta nativamente a verifica��o <RoleProtectedRoute allowedRoles={['comprador']}>. Agora visitantes n�o autenticados n�o conseguem vazar dados B2B da sacola!
2. **Moveu /mercado e /painel-fornecedor:** para o novo grupo (fornecedor), blindando o leil�o reverso contra vazamento.
3. **Refatora��o:** Remo��o das travas locais (useEffects e ifs de loading/user espalhados) centralizando no Cofre Arquitetural do layout.

## Marco (2026-08-20) - Security Patch Cr�tico Aplicado

**Resumo da Sess�o:**
Aplica��o das corre��es mapeadas no elatorio-analise-seguranca.md.

**O que foi feito:**
1. **XSS em E-mails (
otifications.ts):** Adicionada rotina de escapeHTML blindando contra nomes de produtos contendo scripts na renderiza��o para o Resend.
2. **Exposi��o de Secrets (wrangler.jsonc):** Chave ADMIN_UID removida do arquivo p�blico e migrada para .dev.vars (secrets no D1/Workers).
3. **TOCTOU / Overselling (orders.ts):** Adicionado check bloqueante de estoque e db.update at�mico via db.batch no momento da cria��o do pedido. Refatorada a rota de cancelamento para usar RETURNING bloqueando falhas de race condition (TOCTOU) e restaurar o estoque ao cancelar pedido.

---
## Marco (2026-08-20) - Vitrine Exclusiva B2C do Fornecedor

**Resumo da Sess�o:**
Desacoplamento do componente de cat�logo e cria��o de rotas din�micas de isolamento de loja B2C (/catalogo/fornecedor/[id]), atualizando a sidebar do painel B2B para apontar dinamicamente via UID de sess�o do fornecedor logado.

**O que foi feito:**
1. Extra��o de CatalogoContent e useFilters para components/catalogo/CatalogoView.tsx.
2. Cria��o da rota pp/(main)/catalogo/fornecedor/[fornecedorId]/page.tsx.
3. Adapta��o do useFilters para utilizar usePathname() e useParams() do App Router.
4. Altera��o do atalho 'Ver Cat�logo B2C' na SupplierSidebar.tsx.
5. Valida��o total via 27 novos testes unit�rios.
