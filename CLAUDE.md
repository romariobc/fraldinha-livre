...
Sempre que iniciar uma **nova sessão**, ANTES de escrever qualquer código ou executar comandos relativos à solicitação do usuário, **você DEVE LER E EXECUTAR OBRIGATORIAMENTE o roteiro da skill `session-start`** contido no arquivo [.agents/skills/session-start/SKILL.md](file:///.agents/skills/session-start/SKILL.md). Isso garante o alinhamento total de contexto.

Resumo do ciclo (que será executado automaticamente por você após ler a skill):
...
Ao finalizar o trabalho do dia ou quando o usuário solicitar o encerramento da sessão, **você DEVE LER E EXECUTAR OBRIGATORIAMENTE o roteiro da skill `session-finish`** contido no arquivo [.agents/skills/session-finish/SKILL.md](file:///.agents/skills/session-finish/SKILL.md) para empacotar o contexto, validar o código e gerar os commits atômicos antes de se despedir.
